...
(function($) {
    $.fn.geocodify = function(options) {
        ...
        return this.each(function() {
            ...
            $this.onGeocode = function(force) {
                return function(results, status) {
                    ...
                    if (count === 0) {
                        ...
                        // Hide dropdown menu when moving from second to third page
                        var geo_one = $('#geocoder_one-input');
                        var geo_two = $('#geocoder_two-input');
                        // var geo_three = $('#geocoder_three-input');

                        if (geo_two.val() !== '') {
                            // geo_one.val( geo_two.val() );
                            $('#geocoder_one-dropdown').hide();
                        }

                        // else if (geo_three.val() !== '') {
                        //     // geo_one.val( geo_three.val() );
                        //     $('#geocoder_one-dropdown').hide();
                        // }
            ...
            if (button) {
                button.click(function(){$this.fetch(input.val(), true);});
            }
...
function searchKeyPress(e) {
    ...
    if (e.keyCode == 13) {
        ...
        if ( $('#geocoder_one-input').val() !== '' ) {
            document.getElementById('geocoder_one-button').click();  
        } else if ( $('#geocoder_two-input').val() !== '' ) {
            document.getElementById('geocoder_two-button').click();  
        }

        // else if ( $('#geocoder_three-input').val() !== '' ) {
        //     document.getElementById('geocoder_three-button').click();
        // }
    }
}